...
public class AuthenticationController {
    ...
    public String processRegistrationForm(@ModelAttribute @Valid RegisterFormDTO registerFormDTO, Errors errors,
        ...
        if (existingUser != null) {
            errors.rejectValue("username", "username.alreadyExists", "this username is already" +
                    "in use.");
            model.addAttribute("title", "Register");
            return "register";
        }
        String password = registerFormDTO.getPassword();
        String verifyPw = registerFormDTO.getVerifyPassword();
    ...
    @PostMapping("/login")
    public String processLoginForm(@ModelAttribute @Valid LoginFormDTO loginFormDTO, Errors errors, HttpServletRequest
            request, Model model){
        if (errors.hasErrors()) {
            model.addAttribute("title", "Log In");
            return "login";
        ...
        if (!theUser.isMatchingPassword(password)) {
            errors.rejectValue("password", "password.invalid", "The username/password" +
